...
raw_csv = """メールアドレス,氏名,氏,名,しめい,,みょうじ,なまえ,回生,部署,学籍番号,学部,学科,学年,生年月日, photo_url
...
def kana_to_romaji(kana):
    ...
    while i < len(kana):
        ...
        one = kana[i]
        if one == 'っ':
            next_two = kana[i+1:i+3]
            r_next = kana_map.get(next_two, kana_map.get(kana[i+1:i+2], ''))
            if r_next:
                res += r_next[0]
            else:
                res += 'xtsu'
...
for idx in range(1, len(lines)):
    ...
    if len(row) < 15:
        continue
    email = row[0].strip()
    name_kanji = row[1].strip()
    last_kanji = row[2].strip()
    first_kanji = row[3].strip()
    kana_full = row[4].strip().replace(' ', '').replace('　', '')
    myouji_kana = row[6].strip().replace(' ', '').replace('　', '')
    namae_kana = row[7].strip().replace(' ', '').replace('　', '')
    grade = int(row[8].strip()) if row[8].strip().isdigit() else 1
    dept = row[9].strip()
    student_id = row[10].strip()
    faculty = row[11].strip()
    department = row[12].strip()
    birth = row[14].strip()
    photo_url = row[15].strip() if len(row) > 15 else ''

    kana_spaced = f"{myouji_kana} {namae_kana}"
    romaji = f"{kana_to_romaji(myouji_kana)} {kana_to_romaji(namae_kana)}"
 ...
 */
window.INITIAL_MEMBERS = {json.dumps(members, ensure_ascii=False, indent=2)};
...
with open('js/mock-data.js', 'w', encoding='utf-8') as f:
    f.write(mock_js)

print(f"Generated data for {len(members)} members.")
